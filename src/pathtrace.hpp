...
bool render(std::mutex& mutex,
            std::vector<u_char>& row_image,
  ...
  for (int iy = 0; iy < info.render_size.y(); ++iy) {
    std::vector<Pixel> image(info.render_size.x());
    for (int ix = 0; ix < info.render_size.x(); ++ix) {
      Pixel sub_pixel = Pixel::Zero();
      for (int i = 0; i < info.subpixel_num; ++i) {
        Real x = ix + info.render_start.x();
        Real y = iy + info.render_start.y();
...

    {
      ...
      int index = (iy + info.render_start.y()) * info.size.x() * 3 + info.render_start.x() * 3;
